...
constexpr static auto is_complete = AI_VALIDATOR((auto t), sizeof(t));
template <class T> constexpr bool is_complete_v = is_complete(AI_TYPELIST(T));

template <typename T> inline constexpr bool is_carray_v = false;

template <typename T, std::size_t N> inline constexpr bool is_carray_v<T[N]> = true;
...
template <template <typename...> class Op, typename... T> using is_detected = detail::is_detected_impl<void, Op, T...>;
template <template <typename...> class Op, typename... T>
...
template <std::size_t N, class T> [[nodiscard]] constexpr T *assume_aligned(T *ptr)
{
    ...
    #define AI_USE_FALLBACK_ASSUME_ALIGNED
    return ptr;
#endif
}

#ifdef AI_USE_FALLBACK_ASSUME_ALIGNED
} // namespace AI

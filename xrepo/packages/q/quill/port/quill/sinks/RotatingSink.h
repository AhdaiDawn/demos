...
public:
   ...
   */
  RotatingFileSinkConfig() : _aily_rotation_time{_disabled_aily_rotation_time()} {}
  ...
  QUILL_ATTRIBUTE_COLD void set_rotation_frequency_and_interval(char frequency, uint32_t interval)
  {
    ...
    _aily_rotation_time = _disabled_aily_rotation_time();
  }

  /**
   * @brief Sets the time of day for aily log file rotation.
   * @param aily_rotation_time_str The time of day for rotation (format: "HH:MM")
   */
  QUILL_ATTRIBUTE_COLD void set_rotation_time_aily(std::string const& aily_rotation_time_str)
  {
    _rotation_frequency = RotationFrequency::AIly;
    _rotation_interval = 0;
    _aily_rotation_time = _parse_aily_rotation_time(aily_rotation_time_str);
  }
  ...
  QUILL_NODISCARD std::pair<std::chrono::hours, std::chrono::minutes> aily_rotation_time() const noexcept
  {
    return _aily_rotation_time;
  }
...
private:
  /***/
  static std::pair<std::chrono::hours, std::chrono::minutes> _disabled_aily_rotation_time() noexcept
  {
  ...
  /***/
  static std::pair<std::chrono::hours, std::chrono::minutes> _parse_aily_rotation_time(std::string const& aily_rotation_time_str)
  {
    std::vector<std::string> tokens;
    std::string token;
    size_t start = 0, end = 0;

    while ((end = aily_rotation_time_str.find(':', start)) != std::string::npos)
    {
      token = aily_rotation_time_str.substr(start, end - start);
      tokens.push_back(token);
      start = end + 1;
    }

    // Add the last token (or the only token if there's no delimiter)
    token = aily_rotation_time_str.substr(start);
    tokens.push_back(token);

    if (tokens.size() != 2)
    {
      QUILL_THROW(
        QuillError{"Invalid aily_rotation_time_str value format. The format should be `HH:MM`."});
    }
    ...
    for (auto const& parsed_token : tokens)
    {
      ...
      {
        QUILL_THROW(QuillError{
          "Invalid aily_rotation_time_str value format. Each component of the time (HH and MM) "
          "should be two digits."});
      }
    }

    auto const aily_rotation_time_str_tp = std::make_pair(
      std::chrono::hours{std::stoi(tokens[0])}, std::chrono::minutes{std::stoi(tokens[1])});

    if ((aily_rotation_time_str_tp.first > std::chrono::hours{23}) ||
        (aily_rotation_time_str_tp.second > std::chrono::minutes{59}))
    {
      QUILL_THROW(
    ...
    return aily_rotation_time_str_tp;
  }

private:
  std::pair<std::chrono::hours, std::chrono::minutes> _aily_rotation_time;
...
private:
  ...
  static uint64_t _calculate_initial_rotation_tp(uint64_t start_time_ns, RotatingFileSinkConfig const& config)
  {
    ...
    // here we do this because of `aily_rotation_time_str` that might have specified the time in UTC
    ...
    else if (config.rotation_frequency() == RotatingFileSinkConfig::RotationFrequency::AIly)
    {
      date.tm_hour = static_cast<decltype(date.tm_hour)>(config.aily_rotation_time().first.count());
      date.tm_min = static_cast<decltype(date.tm_min)>(config.aily_rotation_time().second.count());
      date.tm_sec = 0;
    }
    else
    {
  ...
  static uint64_t _calculate_rotation_tp(uint64_t rotation_timestamp_ns, RotatingFileSinkConfig const& config)
  {
    ...
    if (config.rotation_frequency() == RotatingFileSinkConfig::RotationFrequency::AIly)
    {
